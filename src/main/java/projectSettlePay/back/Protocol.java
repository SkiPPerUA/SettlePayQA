...
public class Protocol {
    ...
    public Protocol(){
        dataApiPay = new DataBase(DataBase.DataBaseName.APIPAY_STAGE_1);
    ...
    public void protocol_id(String protocol_versions$path) throws SQLException {
        ...
        if (res.next()){
            ...
            dataApiPay.updateSql("insert into public.protocols(created_at, updated_at,name,sandbox_version_id,production_version_id)\n" +
                    "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"','"+protocol_versions$path+"',"+protocol_versions$id+","+protocol_versions$id+")");
    ...
    public void wallet_services(int services$id) throws SQLException {
        dataApiPay.updateSql("insert into public.account_wallet_type_services(created_at, updated_at,type_id, service_id, status)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',1,"+services$id+",1)");
    }
    ...
    public void points_services(int points$id, int services$id) throws SQLException {
        dataApiPay.updateSql("insert into public.point_services(created_at, updated_at,point_id, service_id, status, use_scoring,retry_enable,transparency_token_required,use_antifraud,finmon_enabled)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+points$id+","+services$id+",1,false,false,false,false,false)");
    }

    public void service_commission_lines(int services$id) throws SQLException {
        dataApiPay.updateSql("insert into public.service_commission_lines(created_at, updated_at,template_id,service_id,active)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',1,"+services$id+",true);");
        dataApiPay.updateSql("insert into public.service_commission_lines(created_at, updated_at,template_id,service_id,active)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',2,"+services$id+",true);");
    ...
    public void commission_line_elements(List<Integer> service_commission_lines$ids, boolean pay_in){
        dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_rounding,result_rounding_scale)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+service_commission_lines$ids.get(0)+",'calc_company_reward',0,0,2,'formula',0,4);");
        dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_type,result_rounding,result_rounding_scale)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+service_commission_lines$ids.get(0)+",'buying_price',0,0,1,'customer_amount','final_amount',0,4);");
        dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_type,result_rounding,result_rounding_scale)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+service_commission_lines$ids.get(1)+",'customer',0,0,2,'amount','final_amount',1,2);");
            if (pay_in) {
                dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_type,result_rounding,result_rounding_scale,wallet_from)\n" +
                        "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"'," + service_commission_lines$ids.get(1) + ",'point',0,0,1,'amount','final_amount',0,4,3);");
            }else {
                dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_type,result_rounding,result_rounding_scale,wallet_to)\n" +
                        "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"'," + service_commission_lines$ids.get(1) + ",'point',0,0,1,'amount','final_amount',0,4,3);");
            }
        dataApiPay.updateSql("insert into public.service_commission_line_elements(created_at, updated_at,line_id,type_id,commission_percent,commission_amount,priority,base_type,result_type,result_rounding,result_rounding_scale)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+service_commission_lines$ids.get(1)+",'provider',0,0,3,'amount','final_amount',1,2);");
    }

    public void service_provider_services(int services$id, int provider_services$id){
        dataApiPay.updateSql("insert into public.service_provider_services(created_at, updated_at,service_id,provider_service_id,status,priority)\n" +
                "values ('"+ Data.getCurrentTime() +"','"+ Data.getCurrentTime()+"',"+services$id+","+provider_services$id+",1,1);");
    }
