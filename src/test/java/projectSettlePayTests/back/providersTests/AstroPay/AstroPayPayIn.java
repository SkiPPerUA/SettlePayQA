...
public class AstroPayPayIn extends BaseTest {
    ...
    public void negative_payin_APPROVED_after_CANCELED(){
        ...
        getDataBase(APIPAY_STAGE_1).updateSql("UPDATE public.transactions SET created_at = '"+ Data.getMinusDays(5) +"', updated_at = '"+Data.getMinusDays(5)+"' WHERE id = "+child+"");
        getDataBase(APIPAY_STAGE_1).updateSql("UPDATE public.transactions SET created_at = '"+ Data.getMinusDays(5) +"', updated_at = '"+Data.getMinusDays(5)+"' WHERE id = "+astroPay.getId()+"");
        makeCallback("CANCELED");
    ...
    public void positive_payin_APPROVED_after_fiveDays(){
        ...
        long child = Long.valueOf(astroPay.getId()) + 1;
        //getDataBase(CONN_STAGE_1).updateSql("UPDATE public.transactions SET created_at = '"+ Data.getMinusDays(5) +"', updated_at = '"+Data.getMinusDays(1)+"' WHERE external_transaction_id = '"+child+"'");
        getDataBase(APIPAY_STAGE_1).updateSql("UPDATE public.transactions SET created_at = '"+ Data.getMinusDays(6) +"', updated_at = '"+Data.getMinusDays(6)+"' WHERE id = "+child+"");
        getDataBase(APIPAY_STAGE_1).updateSql("UPDATE public.transactions SET created_at = '"+ Data.getMinusDays(6) +"', updated_at = '"+Data.getMinusDays(6)+"' WHERE id = "+astroPay.getId()+"");
        makeCallback("APPROVED");
        Assert.assertEquals(new TransInfoConn(Long.valueOf(astroPay.getId())+1).getStatus(),2);
        Assert.assertEquals(new TransInfoCore(Long.valueOf(astroPay.getId())).getStatus(),2);
        Assert.assertEquals(new TransInfoCore(Long.valueOf(astroPay.getId())+1).getStatus(),2);
